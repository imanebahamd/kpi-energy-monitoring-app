...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    // Forcer la mise à jour après l'initialisation de la vue
    setTimeout(() => {
  ...
  loadData(): void {
    ...
    this.electricityService.getAnnualSummary(this.selectedYear).subscribe({
      next: (data) => {
        console.log('Données électriques reçues:', data);
    ...
    this.waterService.getMonthlyData(this.selectedYear).subscribe({
      next: (data) => {
        console.log('Données eau reçues:', data);
        if (data && Array.isArray(data) && data.length > 0) {
          this.waterData = data;
  ...
  private updateElectricityChart(): void {
    ...
    console.log('Données électriques mises à jour:', {
      '60KV': newData,
      '22KV': newData22
    });
  }
  ...
  private updateWaterChart(): void {
    ...
    console.log('Données eau mises à jour:', newData);
  }
